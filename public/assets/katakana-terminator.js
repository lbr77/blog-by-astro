...
function translate(phrases) {
    ...
    var api = apiList[0];
    console.log("Katakana Terminator: fetching :",'https://' + api.hosts[0] + api.path + buildQueryString(api.params(phrases)))
    fetch('https://' + api.hosts[0] + api.path + buildQueryString(api.params(phrases)),{
    }).then((response)=>response.json()).then((data)=>{
        api.callback(phrases, data)
...
$("head").append("<style>rt.katakana-terminator-rt::before { content: attr(data-rt); }</style>")
